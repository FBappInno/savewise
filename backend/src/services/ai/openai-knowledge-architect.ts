  ...
  const response =
    await openai.responses.parse({
      ...
      instructions: [
        ...
        "Merge true synonyms, spelling variants, singular/plural variants, translations and semantically equivalent labels into one canonical node.",
        "Examples of labels that should normally become one canonical node when they mean the same thing include Freizeit / Freizeitaktivität, military technology / Militärtechnik, and equivalent singular/plural forms.",
        "Store every merged original galaxy label exactly as an alias of the canonical domain node.",
        "Aliases are used by SaveWise to permanently normalize discovery classifications after an explicit universe synchronization.",
        "Only use aliases for labels that are genuinely semantically equivalent.",
        "Do not treat a broader category as an alias: for example Shopping must not become an alias of Lifestyle, and Robotik must not become an alias of Technologie.",
        "Prefer one concise canonical label in the dominant language of the user's library.",
        "Store the removed variants as aliases of the canonical node.",
        "Do not merge merely related concepts: Reisen and Freizeit may belong to the same semantic cluster while remaining separate galaxies.",
        ...
        "A discovery's supplied classification is the authoritative SaveWise hierarchy.",
        "Always map classification.secondaryCategory to the domain/galaxy, classification.topic to the topic/planet, and classification.subtopics to subtopic/star nodes.",
        "Never move a discovery into a different domain, topic or subtopic than its stored classification.",
        "This rule applies to both AI-generated and manually selected classifications.",
        "classification.mode only records whether the classification came from AI or the user; it does not change the authority of the stored hierarchy.",
        "You may create semantic relations, aliases and cross-links between nodes, but those relations must never replace or change the stored Galaxie -> Planet -> Sterne path.",
        "You may merge spelling variants or true synonyms only when doing so does not change the semantic meaning of the stored classification.",
